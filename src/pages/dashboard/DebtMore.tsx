...
const DebtMore = () => {
  ...
  return (
    <div className='containers space-y-[30px]'>
      {isLoading ? (<div>
        <Skeleton.Node active style={{width: '380px', marginTop: '30px', height: '40px'}}/>
        <Skeleton.Node active style={{width: '380px', marginTop: '30px', height: '100px'}}/>
        <Skeleton.Node active style={{width: '380px', marginTop: '30px', height: '300px'}}/>
        <Skeleton.Node active style={{width: '380px', marginTop: '30px', height: '200px'}}/>
        </div>
       ) : (
        <div>
          ...
          <div className='space-y-[24px]'>
            <div className='flex gap-[20px]'>
              ...
              <div className='w-[30%] space-y-[5px]'>
                <Heading classList='!text-[16px] !font-sans !font-medium' tag='h3'>Sana</Heading>
                <div className='bg-[#F6F6F6] border-[1.5px] border-[#ECECEC] rounded-[5px] py-[8px] px-[12px]'>{debt?.updatedAt.split('T')[1].substring(0, 5)}</div>
              </div>
            </div>
            ...
            <div className='mb-[10px]'><CustomButton onClick={() => {navigate("sondirish"); queryClient.invalidateQueries({queryKey: ["debt-sondirish"]})}} isDisabled={false} classList='!mt-[10px]'>Nasiyani so‘ndirish</CustomButton></div>
          </div>
          {modal && <CustomModal url={`/debt/${debt?.id}`} open={modal} queryKey="debt-more" setOpen={setModal}/>}
        </div>
      )}
    </div>
  )
